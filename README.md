...
<img src="docs/screenshots/app-overview.png" alt="Редактор помещения и смета с вилкой стоимости" width="100%">
...
- ✏️ **2D-редактор помещения** — многоугольник любой формы, несколько комнат,
  высота потолка, форма потолка (плоский / многоуровневый / мансардный скат),
  двери и окна.
- 📐 **Точная геометрия** — площадь пола и потолка, периметр, площадь стен за
  вычетом проёмов; расчёт в `Decimal`, площадь по формуле шнурования.
- 🧱 **Материалы по нормам** — расход, округление вверх до фасовки, запас 8–12 %.
- 👷 **Работы специалистов** — стоимость по операциям с делением на черновые,
  предчистовые и чистовые этапы.
- 🎚️ **Объём ремонта** — только чистовая / черновая + чистовая / только черновая.
- 💰 **Вилка цены** — минимум / средняя / максимум отдельно по материалам и работам,
  а для ключевых позиций — варианты «Эконом / Стандарт / Премиум».
- 🔍 **Прозрачные цены** — у каждой строки источник (ссылка на карточку товара или
  прайс компании), регион и дата обновления; выбор магазина (Мегастрой / Леман).
- 🕳️ **Скрытые работы** — отдельная вилка возможных доплат за то, что вскрывается
  только на объекте (демонтаж, выравнивание, стяжка, штробление).
- 💾 **Проекты** — сохранение сметы и ссылка «поделиться».
- 📄 **Экспорт** — PDF, Excel, печать сметы (кириллица, раскрытый состав).
...
2. Указываете высоту и форму потолка, двери и окна.
3. Выбираете объём ремонта (только чистовая / черновая + чистовая / только
   черновая), город и магазин, а также набор работ и отделку по каждой
   поверхности (пол, стены, потолок, электрика, сантехника).

→ Система считает площади, материалы и работы и собирает детальную смету с
вилкой стоимости и ссылками на реальные источники цен.
