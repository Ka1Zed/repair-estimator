...
class DBSettings(BaseSettings):
    ...
    # PRICE_STALE_TTL_HOURS — отдаём её вместо общей seed-цены (реальная цена
    # вчера/на прошлой неделе точнее общего seed). Старше — считаем, что парсер
    # сломан надолго, и уходим в seed. По умолчанию неделя.
    PRICE_STALE_TTL_HOURS: int = 24 * 7
