...
LABOR_SERVICE_MAP = {
    ...
    # Нишевые услуги (#407): раньше жили только на seed-цене (одна строка в БД,
    # вилка держалась на глазок). Роутим их в прайсы подрядчиков, чтобы коридор
    # min/avg/max опирался на 3+ реальных источника.
    "Грунтование потолка": {
        # Потолочный грунт (#380) — отдельная услуга от «Грунтование» (стены/пол),
        # та явно исключает "потолк"; здесь include_all "потолк" их разводит.
        "include_all": ["потолк"],
        "include_any": ["грунтован", "грунтовк"],
        "exclude": ["демонтаж", "багет", "короб", "антигриб"],
        "unit": "м²",
    },
    "Шпаклевка потолка": {
        # Потолочная шпаклёвка (#380) — по аналогии с «Шпаклевка стен» (та требует
        # "стен"); здесь include_all "потолк".
        "include_all": ["потолк"],
        "include_any": ["шпатлевк", "шпаклевк", "шпатлёвк", "шпаклеван", "шпатлеван"],
        # "торцов"/"ригел"/"балок" — кромки двухуровневых потолков, грани ригелей и
        # балок: тарифицируются за пог. м проёма/грани, не за м² плоскости (та же
        # причина, что и вынос откосов из скоупа, #407). Единичный фильтр их и так
        # роняет (unit "м"), но у сайта без чистой ячейки-единицы _unit_matches(None)
        # пропускает строку — exclude страхует, чтобы дорогой погонный тариф не утёк
        # в вилку м² и не завышал max (#410).
        "exclude": ["демонтаж", "очистк", "армирован", "торцов", "ригел", "балок",
                    "наценк"],
        "unit": "м²",
    },
    "Штробление": {
        "include_all": [],
        "include_any": ["штроблен", "устройство штробы"],
        # chasing (hidden_works_service) — штроба под кабель, пог. м. Крупные каналы
        # 50×50 мм и больше (под трубы/вентиляцию) — другая по цене операция ×5-10,
        # ломают вилку; "заделка"/"засыпка" штробы — это её закрытие, не резка.
        "exclude": ["50×50", "75×75", "110×110", "заделка", "засыпк", "прокладка",
                    "монтаж", "установка подрозет", "устранени", "щит", "фреонов",
                    "гнезд", "ниш", "труб", "кабел", "без штроблен"],
        "unit": "м",
    },
    "Ниша под карниз": {
        "include_all": [],
        # Ниша/короб/потолок из ГКЛ под скрытый карниз или закарнизную подсветку
        # (curtain_niche, пог. м). НЕ монтаж самой гардины/карниза и не ниша радиатора.
        "include_any": ["закарниз", "ниш", "короб"],
        "include_any2": ["карниз"],
        "exclude": ["демонтаж", "радиатор", "штор", "багет"],
        "unit": "м",
    },
    "Закладная под светильник": {
        "include_all": [],
        "include_any": ["закладн"],
        # ceiling_embed (шт) — устройство закладной под точку света. Исключаем монтаж
        # люстры/светильника НА готовую закладную и закладную под коллекторный шкаф.
        "exclude": ["демонтаж", "коллектор", "люстры на закладн", "подключени",
                    "кольцо", "профиль", "багет"],
        "unit": "шт",
    },
    # «Отделка откосов» (otkos) намеренно НЕ добавлена: в нашей модели откос —
    # это площадь (otkos_area, м², geometry_service), а подрядчики почти всегда
    # тарифицируют откосы за ПОГОННЫЙ метр проёма (unit "м"). Единицы несовместимы
    # (цена/пог.м ≠ цена/м² без деления на глубину), поэтому живые источники дали бы
    # мусорную вилку. Оставлена на seed-коридоре (400/600/900). См. docs/price-sources.md.
}
