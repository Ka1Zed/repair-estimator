...
# Карта: материал в БД -> URL категории, сужённый фасетом «назначение» (#319).
# Общая категория kraski-dlya-sten-i-potolkov мешала стены и потолки (~2500 позиций,
# вилка 30→9672, ~320×) и обе строки указывали на один URL. Фасет ?00277= разводит
# выборки: Стена — краска для стен, Потолок — потолочная (аналог field142[] у
# Мегастроя). Значение фасета — кириллицей как в адресной строке; браузерный фетч
# (patchright) сам его perc-энкодит при переходе, отдельное кодирование не нужно.
# Отдельного глобального фильтра «в наличии» у Лемана нет — «наличие в магазинах»
# это чекбоксы по конкретным магазинам (мск-регион), к Казани неприменимы, поэтому
# не добавляем; семантический мусор (колеры/пробники/грунты) отсекаем по имени ниже.
_PAINT_CATEGORY = "https://kazan.lemanapro.ru/catalogue/kraski-dlya-sten-i-potolkov/"
CATEGORY_MAP = {
    "Краска для стен": f"{_PAINT_CATEGORY}?00277=Стена",
    "Краска потолочная": f"{_PAINT_CATEGORY}?00277=Потолок",
}
