...
class MaterialTierItem(BaseModel):
    ...
    значениями строки. Потребность (base×waste) общая с родительской строкой (не
    пересчитывается по норме расхода конкретного SKU), но фасовка КОНКРЕТНОГО товара
    этого уровня своя (#349): краска стандарт 9 л против премиум 10 л, плинтус 2.5 м
    против 3 м. Поэтому package_size/packs/quantity считаются по своему SKU, и итог
    (quantity округлён вверх до своих упаковок) свой.
    """
    name: str
    price: float
    total: float
    ...
    # Фасовка/единица/упаковки ИМЕННО этого SKU уровня (#349): у finish_key-позиций
    # разные товары → разная фасовка, и фронт обязан показывать её, а не общую из
    # родительской строки (иначе стандарт и премиум оба «× 9 л»). Для no-price/no-SKU
    # — дефолты. quantity — потребность, округлённая вверх до упаковок ЭТОГО товара.
