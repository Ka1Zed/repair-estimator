...
def _selections(repair_options: Dict[str, Any], geom: Dict[str, Any]) -> List[tuple]:
    ...
    reveal_area  = D(geom.get("reveal_area", 0))   # площадь откосов

    # Эффективная площадь стен для отделочных материалов (включая откосы)
    effective_wall_area = wall_area + reveal_area
    ...
    # --- стены ---
    if walls == "paint":
        sel.append((M_PRIMER, effective_wall_area))
        sel.append((M_PUTTY, effective_wall_area))
        sel.append((M_PAINT_WALLS, effective_wall_area))
    elif walls == "wallpaper":
        # для обоев откосы обычно не оклеиваются, оставляем wall_area
    ...
    # ceiling == "stretch" — отдельный блок, не здесь
