...
class TestMaterialCalc:
    ...
    def test_material_calc_rectangle(self, db_session):
        ...
        def test_reveal_material_present(self, db_session):
            """Проверка, что при наличии откосов в списке материалов появляется позиция «Откосы»."""
            geometry = {
                'floor_area': Decimal('12.0'),
                'ceiling_area': Decimal('12.0'),
                'wall_area': Decimal('34.1'),
                ...
                'reveal_area': Decimal('1.645')   # симулируем откосы
            }
            repair_options = {
                'floor': 'laminate',
                'walls': 'paint',
            ...
            materials = calculate_materials(geometry, repair_options, db_session)
            # Ищем материал с именем "Откосы" (или M_REVEAL)
            reveal = next((m for m in materials if m['name'] == 'Откосы'), None)
            assert reveal is not None
            assert reveal['quantity'] == geometry['reveal_area']
            # Проверяем, что есть материал для откосов
...
class TestLaborCalc:
    ...
    def test_labor_calc_rectangle(self, db_session):
        ...
        def test_reveal_labor_present(self, db_session):
            """Проверка, что при наличии откосов в списке работ появляется позиция «Отделка откосов»."""
            geometry = {
                'floor_area': Decimal('12.0'),
                'ceiling_area': Decimal('12.0'),
                'wall_area': Decimal('34.1'),
            ...
            labor = calculate_labor(geometry, repair_options, db_session)
            # Ищем работу с именем "Отделка откосов" (или S_REVEAL)
            reveal_job = next((j for j in labor if j['service'] == 'Отделка откосов'), None)
            assert reveal_job is not None
            assert reveal_job['volume'] == geometry['reveal_area']
