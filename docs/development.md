...
Тесты изолированы от dev-базы: они работают на отдельной БД. `conftest.py` сам
создаёт её при первом запуске, если её ещё нет, и дропает в конце сессии —
отдельная ручная подготовка и чистка не нужны, достаточно поднятого PostgreSQL и
прав на `CREATE DATABASE`. Прогон `pytest` **не затирает** боевой seed в
`repair_estimator`, поэтому повторные запуски подряд не требуют пере-seed.

Тест-база создаётся **per-process**: имя вида `repair_estimator_test_<worker|pidNNN>`
уникально для каждого процесса `pytest`, поэтому два одновременных прогона (два
терминала, локалка + CI на одном Postgres, `pytest -n` под xdist) не мешают друг
другу и не дают ложных падений на общей схеме. Осиротевших `repair_estimator_test_*`
после прогона не остаётся. Явный `POSTGRES_TEST_DB` перекрывает генерацию имени
(фиксированная база для отладки — но тогда межпроцессной изоляции нет, и такую базу
`pytest` за собой не дропает).
...
> **Backend в Docker: пересобирать образ после новых миграций.**
> `docker compose up -d` переиспользует уже собранный образ backend и **не**
> подтягивает свежий код с новыми файлами миграций. Если после `git pull` (или
> своей новой ревизии) поднять контейнер без пересборки, alembic внутри не найдёт
> ревизию из БД и уйдёт в рестарт-луп с ошибкой `Can't locate revision identified by ...`.
> Поэтому после появления новых миграций:
>
> ```bash
> docker compose build backend    # пересобрать образ со свежим кодом
> docker compose up -d            # только теперь поднимать
> ```
