...
- Цены «живут» `PRICE_TTL_HOURS` (по умолчанию 24 ч) — дольше кэш свежий и парсер
  повторно не дёргается. Строка старше TTL, но моложе `PRICE_STALE_TTL_HOURS`
  (по умолчанию месяц) — ещё используется вместо seed, если живой рефетч в этот
  момент не удался (403/VPN/сервер вообще не ходит в сеть, `PARSER_LIVE_FETCH=false`):
  цена парсера месячной давности точнее seed (см. `app/core/config.py`). Старше
  месяца — расчёт уходит на seed-fallback. Никакого кэша в памяти процесса нет: смета
  читает БД свежей сессией на каждый запрос, поэтому после записи цен **рестарт не
  нужен**.
